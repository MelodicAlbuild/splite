...
async function removeemoji(emoji, message, command)
{
  ...
  if (customemoji.id) {
      customemoji.delete().then(()=>{
        message.channel.send({embeds: [new Discord.MessageEmbed().setDescription(`${_emojis.success} ${emoji} Removed!`).setFooter(message.member.displayName, message.author.displayAvatarURL({dynamic: true}))]});
      })

      await command.sendModLogMessage(message, '', {Member: message.member.toString(), 'Removed Emoji': `\`${emoji}\``});
  }
